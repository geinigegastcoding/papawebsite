...
# Optional: extra keys, only if you have them.
npx wrangler versions secret put OPENROUTER_API_KEY_2
npx wrangler versions secret put OPENROUTER_API_KEY_3
```

OpenRouter free means that the selected model endpoint is priced at zero; you still need an OpenRouter account and API key, and free endpoints are rate-limited. The route uses one primary and up to three free multimodal fallbacks: Qwen, Gemma 4 31B, Gemma 4 26B, and OpenRouter's dynamic `openrouter/free` router. OpenRouter also fails over between providers for the same model. `OPENROUTER_MODEL` and the comma-separated `OPENROUTER_MODELS` override the order; keep no more than three backup entries. Free-model availability and provider retention can change, so the model is configurable and the UI labels every photo result as an estimate.

If a separate `OPENROUTER_API_KEY_2` or `OPENROUTER_API_KEY_3` is configured, the Worker tries it only after a rate limit, provider error, or invalid key. Extra keys do not guarantee a higher account-wide free quota, so the normal protection is model/provider fallback rather than repeated duplicate requests.
...
The photo route sends the selected image as base64 to OpenRouter server-side, never to the browser with an API key. If a model returns HTTP success but unusable JSON, the Worker tries the next model instead of blaming the photo. The barcode route requests only the needed fields from Open Food Facts v3, identifies the app with a User-Agent, and keeps the source URL visible so a product can be checked.
